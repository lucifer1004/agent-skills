...
class ClaudeAgentSDKProvider:
    ...
    def _install_skills(
        ...
        for index, source_path in enumerate(skill_paths, start=1):
            source_dir = _resolve_skill_directory(source_path)
            target_dir = _unique_skill_target(skills_root, _skill_dir_name(source_dir, index))
...
def _resolve_skill_directory(source_path: Path) -> Path:
...
def _skill_dir_name(skill_dir: Path, index: int) -> str:
...
def _unique_skill_target(skills_root: Path, base_name: str) -> Path:
