...
def parse_and_validate_json_output(
    ...
    if not candidate:
        raise AgentRuntimeError("structured_output_mismatch", "Structured output was empty.")
...
def _validate_schema(payload: object, schema: dict[str, object], *, path: str) -> None:
    ...
    if schema_type == "string":
        if not isinstance(payload, str):
            raise AgentRuntimeError("structured_output_mismatch", f"{path} must be a string.")
        _validate_enum(payload, schema, path=path)
        return
    if schema_type == "boolean":
        if not isinstance(payload, bool):
            raise AgentRuntimeError("structured_output_mismatch", f"{path} must be a boolean.")
        return
    if schema_type == "integer":
        if isinstance(payload, bool) or not isinstance(payload, int):
            raise AgentRuntimeError("structured_output_mismatch", f"{path} must be an integer.")
        minimum = schema.get("minimum")
        maximum = schema.get("maximum")
        if isinstance(minimum, int) and payload < minimum:
            raise AgentRuntimeError("structured_output_mismatch", f"{path} must be >= {minimum}.")
        if isinstance(maximum, int) and payload > maximum:
            raise AgentRuntimeError("structured_output_mismatch", f"{path} must be <= {maximum}.")
        return
...
def _validate_object(payload: object, schema: dict[str, object], *, path: str) -> None:
    ...
    if not isinstance(payload, dict):
        raise AgentRuntimeError("structured_output_mismatch", f"{path} must be an object.")
    ...
    if not isinstance(properties, dict):
        raise AgentRuntimeError(
            "structured_output_mismatch",
            f"{path} object schema is missing properties.",
        )
    if not isinstance(required, list):
        raise AgentRuntimeError(
            "structured_output_mismatch",
            f"{path} object schema has invalid required list.",
        )
    ...
    for key in required:
        if not isinstance(key, str):
            raise AgentRuntimeError(
                "structured_output_mismatch",
                f"{path} object schema has non-string required key.",
            )
        if key not in payload:
            raise AgentRuntimeError("structured_output_mismatch", f"{path}.{key} is required.")
    ...
    if additional_properties is False:
        ...
        if extra_keys:
            raise AgentRuntimeError(
                "structured_output_mismatch",
                f"{path} has unexpected properties: {', '.join(extra_keys)}.",
            )
...
def _validate_array(payload: object, schema: dict[str, object], *, path: str) -> None:
    ...
    if not isinstance(payload, list):
        raise AgentRuntimeError("structured_output_mismatch", f"{path} must be an array.")

    item_schema = schema.get("items")
    if not isinstance(item_schema, dict):
        raise AgentRuntimeError(
            "structured_output_mismatch",
            f"{path} array schema is missing items.",
        )
...
def _validate_enum(payload: str, schema: dict[str, object], *, path: str) -> None:
    ...
    if not isinstance(enum, list) or not all(isinstance(item, str) for item in enum):
        raise AgentRuntimeError("structured_output_mismatch", f"{path} schema has invalid enum.")
    if payload not in enum:
        raise AgentRuntimeError(
            "structured_output_mismatch",
            f"{path} must be one of: {', '.join(enum)}.",
        )
