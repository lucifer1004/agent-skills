...
def test_codex_runtime_reads_output_last_message_and_usage(monkeypatch):
    case = ResolvedCase(
        ...
        case=BenchmarkCase(
            schema_version=1,
            id="uiux.generate.codex-runtime",
            title="Codex Case",
    ...
    response = CodexCLIAgentRuntime(instructions="Be precise").run(build_candidate_run_spec(case))
